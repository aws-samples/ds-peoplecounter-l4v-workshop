...
const int LookoutVisionInferenceClient::POLLING_TIMEOUT_IN_SECONDS = 180;
...
void LookoutVisionInferenceClient::setServerUrl(std::string server_url) {
    ...
    stub = AWS::LookoutVision::Edge::InferenceServer::NewStub(channel);
}
...
GstLookoutVisionResult* LookoutVisionInferenceClient::DetectAnomalies(std::string model_component, guint8* buf,
    ...
    AWS::LookoutVision::Edge::DetectAnomaliesRequest request;
    AWS::LookoutVision::Edge::DetectAnomaliesResponse reply;
...
LookoutVisionInferenceClient::OperationStatus LookoutVisionInferenceClient::StartModel(std::string model_component) {
    AWS::LookoutVision::Edge::StartModelRequest request;
    AWS::LookoutVision::Edge::StartModelResponse reply;
    grpc::ClientContext context;
    ...
    if (waitForModelStatusWithTimeout(model_component, AWS::LookoutVision::Edge::ModelStatus::RUNNING,
                                      POLLING_TIMEOUT_IN_SECONDS)) {
        return OperationStatus::SUCCESSFUL;
    } else {
        std::cout << "Model didn't reach RUNNING state within "
        << POLLING_TIMEOUT_IN_SECONDS << " seconds" << std::endl;
    }

    return OperationStatus::FAILED;
}

LookoutVisionInferenceClient::OperationStatus LookoutVisionInferenceClient::StopModel(std::string model_component) {
    AWS::LookoutVision::Edge::StopModelRequest request;
    AWS::LookoutVision::Edge::StopModelResponse reply;
    grpc::ClientContext context;

    try {
        request.set_model_component(model_component);
        grpc::Status status = stub->StopModel(&context, request, &reply);

        if (!status.ok()) {
            std::cout << "StopModel returned error "
            << status.error_code() << ": " << status.error_message() << std::endl;
        }
    } catch (std::exception& e) {
        std::cout << "Exception: " << e.what() << std::endl;
    ...
    if (waitForModelStatusWithTimeout(model_component, AWS::LookoutVision::Edge::ModelStatus::STOPPED,
                                      POLLING_TIMEOUT_IN_SECONDS)) {
        return OperationStatus::SUCCESSFUL;
    } else {
        std::cout << "Model didn't reach STOPPED state within "
        << POLLING_TIMEOUT_IN_SECONDS << " seconds" << std::endl;
    }
...
bool LookoutVisionInferenceClient::waitForModelStatusWithTimeout(std::string model_component,
                                                                 AWS::LookoutVision::Edge::ModelStatus expected_status,
                                                                 int timeout_in_seconds) {
    std::time_t start_time = std::time(NULL);

    while (std::time(NULL) - start_time < timeout_in_seconds) {
        AWS::LookoutVision::Edge::ModelStatus* model_status = getModelStatus(model_component);
...
AWS::LookoutVision::Edge::ModelStatus* LookoutVisionInferenceClient::getModelStatus(std::string model_component) {
    AWS::LookoutVision::Edge::DescribeModelRequest request;
    AWS::LookoutVision::Edge::DescribeModelResponse reply;
    grpc::ClientContext context;
    AWS::LookoutVision::Edge::ModelStatus* model_status = NULL;
    ...
    try {
        ...
        if (status.ok()) {
            model_status = new AWS::LookoutVision::Edge::ModelStatus{reply.model_description().status()};
        } else {
