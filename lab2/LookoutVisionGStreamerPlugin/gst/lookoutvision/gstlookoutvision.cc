 ...
 *
 * Performs inference on frames using the Lookout for Vision inference server
 *
 * <refsect2>
 * <title>Example launch line</title>
 * |[
 * gst-launch -v -m fakesrc ! lookoutvision ! fakesink silent=TRUE
 * ]|
 * </refsect2>
 */
...
static void gst_lookout_vision_init(GstLookoutVision * filter) {
    ...
    filter->server_url = g_strdup("unix-abstract:aws.lookoutvision.inference-server");
    try {
        filter->inference_client = new LookoutVisionInferenceClient(filter->server_url);
    } catch (std::runtime_error& e) {
        GST_ELEMENT_ERROR(filter, LIBRARY, FAILED, (NULL), ("%s", e.what()));
    }
}
    ...
    switch (prop_id) {
        case PROP_SERVER_URL:
            ...
            if (filter->model_component) {
                if (filter->inference_client->StartModel(filter->model_component)
